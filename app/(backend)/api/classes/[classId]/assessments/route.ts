...
export async function GET(req: Request, { params }: { params: { classId: string } }) {
  try {
    ...
    const formattedAssessments = Object.values(groupedAssessments).map((assessment: any) => {
      ...
      const averageScore = scores.length
        ? Number((scores.reduce((acc: number, curr: number) => acc + curr, 0) / scores.length).toFixed(1))
        : 0
    ...
    const overallAverage = allScores.length
      ? Number((allScores.reduce((acc, curr) => acc + curr.score, 0) / allScores.length).toFixed(1))
      : 0
    ...
    const distribution = distributionRanges.map((range) => {
      ...
      return {
        grade: range.grade,
        count,
        percentage: allScores.length ? Math.round((count / allScores.length) * 100) : 0,
      }
    })
